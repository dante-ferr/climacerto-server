import { BaseHTTPError } from "../../shared/custom-erros/base-http-error.js"
...
export class WeatherService implements IWeatherService {
    ...
    async getWeatherByCoords(latitude: string, longitude: string, date: string): Promise<ClimateData> {
        ...
        if (!latitude || !longitude) {
            throw new BadRequestError("Os parâmetros 'longitude' e 'latitude' são obrigatórios.")
        }
        ...
        if (!response.ok) {
            throw new ServiceUnavailableError("Não foi possível obter os dados climáticos no momento.")
        }
    ...
    async getWeatherByName(name: string, date: string): Promise<ClimateData> {
        ...
        if (!response.ok) {
            throw new ServiceUnavailableError("Não foi possível obter os dados de geolocalização no momento.")
        }
